...
class Mangle(_mangle.Mangle):
    ...
    def polyid_and_weight(self, ra, dec):
        ...
        """
        ra = array(ra, ndmin=1, dtype=np.longdouble, copy=False, order='C')
        dec = array(dec, ndmin=1, dtype=np.longdouble, copy=False, order='C')
    ...
    def check_quadrants(self,
        ...
        dec = array(dec, ndmin=1, dtype=np.longdouble, copy=False, order='C')
        angle_degrees = array(
            angle_degrees, ndmin=1, dtype=np.longdouble, copy=False, order='C'
        )
    ...
    def calc_simplepix(self, ra, dec):
        ...
        Array of zeros or ones
        """
        ra = array(ra, ndmin=1, dtype=np.longdouble, copy=False)
        dec = array(dec, ndmin=1, dtype=np.longdouble, copy=False)
    ...
    def _set_weights(self, weights):
        ...
        # make long doubles
        weights = array(weights, ndmin=1, dtype=np.longdouble, copy=False)
...
class Cap(_mangle.Cap):
    ...
    def set(self, data):
        ...
        data: array or sequence
            An length 4 array of 128 bit floats, or convertable to that.
        """
        data = array(data, ndmin=1, dtype=np.longdouble, copy=False)
        if data.size != 4:
            raise ValueError(
...
class Polygon(_mangle.Polygon):
    ...
    def __init__(self, poly_id, pixel_id, weight, cap_vec):
        ...
        wtarr = array(weight, ndmin=1, dtype=np.longdouble, copy=False)
